...
let deleteApigResource = async(derpConfigData, routeName) => {
    let params = {
        resourceId: derpConfigData.routes[routeName].gateway.resource.id,
        restApiId: derpConfigData.gateway.id,
    };

    util.showLog(`Deleting API Gateway resource...`);
    await apig.deleteResource(derpConfigData, params)
        .catch(catchPromiseError.bind(this));
}

let deleteLambdaFunction = async(derpConfigData, routeName) => {
    let params = {
        FunctionName: derpConfigData.routes[routeName].lambda.functionName
    };

    util.showLog(`Deleting lambda function...`);
    await lambda.deleteFunction(derpConfigData, params)
        .catch(catchPromiseError.bind(this));
}

let ensureLambdaRolePolicyUndeployed = async function(derpConfigData) {
    util.showLog(`Deleting Lambda Role Policy ${derpConfigData['lambda-role'].roleName}...`);

    let params = {
        PolicyName: derpConfigData['lambda-role'].roleName,
        RoleName: derpConfigData['lambda-role'].roleName
    }

    await iam.deleteIAMRolePolicy(derpConfigData, params)
        .catch(catchPromiseError.bind(this));
}

let ensureLambdaRoleUndeployed = async function(derpConfigData) {
    util.showLog(`Deleting Lambda Role ${derpConfigData['lambda-role'].roleName}...`);

    let params = {
        RoleName: derpConfigData['lambda-role'].roleName
    }

    await iam.deleteIAMRole(derpConfigData, params)
        .catch(catchPromiseError.bind(this));
}

let ensureApiGatewayUndeployed = async function(derpConfigData) {
    util.showLog(`Deleting API Gateway ${derpConfigData.gateway.id}...`);

    let params = {
        restApiId: derpConfigData.gateway.id
    }

    await apig.deleteRestApi(derpConfigData, params)
        .catch(catchPromiseError.bind(this));
}
...
module.exports = {
    ...
    ensureLambdaUndeployed: async function(derpConfigData, routeName) {
        util.showLog(`Undeploying ${routeName}...`);

        await deleteApigResource(derpConfigData, routeName);
        await deleteLambdaFunction(derpConfigData, routeName);

        delete derpConfigData.routes[routeName].gateway;
        delete derpConfigData.routes[routeName].lambda;
        delete derpConfigData.routes[routeName].url;
        util.updateDerpConfigFileData(derpConfigData);

        return true;
    },

    removeUnusedResources: async function(derpConfigData) {
        util.showLog(`Undeploying unused resources...`);

        await ensureLambdaRolePolicyUndeployed(derpConfigData);
        await ensureLambdaRoleUndeployed(derpConfigData);
        await ensureApiGatewayUndeployed(derpConfigData);

        delete derpConfigData['lambda-role'];
        delete derpConfigData.gateway;
        util.updateDerpConfigFileData(derpConfigData);
    }
};