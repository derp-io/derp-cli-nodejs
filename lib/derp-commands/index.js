...
exports.undeployRoute = async function(routeName) {
    ...
    if (!util.isRouteDeployed(derpConfigData, routeName)) {
        util.showLog(`${routeName} is not deployed.`);
        return false;
    } else {
        await awsInterface.ensureLambdaUndeployed(derpConfigData, routeName);

        if (!util.isAnyRouteDeployed(derpConfigData)) {
            //clean up if all routes undeployed
            await awsInterface.removeUnusedResources(derpConfigData);
        }
