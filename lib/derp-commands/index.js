...
exports.deployRoute = function (routeName) {
    ...
    awsInterface.createOrUpdateLambda(derpConfigData, route, routeName,
        (derpConfigData) => {
            awsInterface.createOrUpdateAPIG(derpConfigData, route, routeName);
        });
};

exports.testRoute = function (routeName) {
    let derpConfigData = util.getDerpConfigFileData();
    let route = derpConfigData.routes[routeName];
    util.showLog(`Testing route found at ${route.path}.`);

    let fullRoutePath = resolve(__dirname, `../../routes/${routeName}/index.js`);
