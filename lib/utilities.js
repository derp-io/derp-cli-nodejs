...
module.exports = {
    ...
    createLambdaRoleInlinePolicy: function(derpConfigData, cb) {
        ...
        iam.putRolePolicy(iamPolicyParams)
            .on('retry', (response) => {
                if (response.error) {
                    this.showLog(`Retrying lambda role policy association...`);
            ...
            .on('success', (data) => {
                this.showLog(`Added Inline Policy For Lambda Role`);
    ...
    createLambdaFunction: function(derpConfigData, routeName, cb) {
        if (derpConfigData.routes[routeName].lambda &&
            ...
            lambda.createFunction(this.setupNewLambdaParams(derpConfigData, routeName))
                .on('retry', (response) => {
                    if (response.error) {
                        this.showLog('Waiting for Lambda role to associate new policy...');
                        response.error.retryable = true; // retry this error
                        response.error.retryDelay = 4000; // wait 4 seconds
                    }
                })
                .on('error', (err) => {
